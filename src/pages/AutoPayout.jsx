...
const Toggle = ({ enabled, onChange }) => (
  <button
    onClick={() => onChange(!enabled)}
    className={`relative inline-flex h-5 w-9 sm:h-6 sm:w-11 items-center rounded-full transition-colors duration-200 focus:outline-none ${enabled ? "bg-green-500" : "bg-gray-300"}`}
  >
    <span
      className={`inline-block h-3.5 w-3.5 sm:h-4 sm:w-4 transform rounded-full bg-white shadow transition-transform duration-200 ${enabled ? "translate-x-5 sm:translate-x-6" : "translate-x-0.5 sm:translate-x-1"}`}
    />
  </button>
);

const StatusBadge = ({ active }) => (
  <span className={`px-1.5 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-semibold border ${active ? "bg-green-50 text-green-700 border-green-200" : "bg-gray-100 text-gray-500 border-gray-200"}`}>
    {active ? "Active" : "Inactive"}
  </span>
);
...
const SearchInput = ({ placeholder }) => (
  <div className="relative w-full sm:w-auto">
    <input
      className="w-full pl-7 sm:pl-8 pr-7 sm:pr-8 py-1.5 border border-gray-200 rounded-lg text-[11px] sm:text-xs text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400"
      placeholder={placeholder}
    />
    <svg className="absolute left-2 sm:left-2.5 top-1/2 -translate-y-1/2 text-gray-400" width={11} height={11} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
      <circle cx="11" cy="11" r="8" /><path d="M21 21l-4.35-4.35" />
    </svg>
  </div>
);
...
const TableSection = ({ title, searchPlaceholder, headers, rows, onToggle }) => (
  <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 overflow-hidden mb-4 sm:mb-5">
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 sm:px-5 py-3 sm:py-4 border-b border-gray-100">
      <h2 className="text-xs sm:text-sm font-bold text-gray-900">{title}</h2>
      <SearchInput placeholder={searchPlaceholder} />
    </div>
    <div className="overflow-x-auto">
      <table className="w-full min-w-[1000px]">
        <thead>
          <tr className="bg-gray-50/60 border-b border-gray-100">
            {headers.map((h) => (
              <th key={h} className="px-3 sm:px-4 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-semibold text-gray-500 whitespace-nowrap">{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
              <td className="px-3 sm:px-4 py-2.5 sm:py-3.5 text-[11px] sm:text-xs font-medium text-gray-800 whitespace-nowrap">{row.col1}</td>
              <td className="px-3 sm:px-4 py-2.5 sm:py-3.5 text-[11px] sm:text-xs font-medium text-gray-800 whitespace-nowrap">{row.col2}</td>
              <td className="px-3 sm:px-4 py-2.5 sm:py-3.5">
                <div className="flex items-center gap-1.5 sm:gap-2.5">
                  <Toggle enabled={row.enabled} onChange={(v) => onToggle(i, v)} />
                  <span className={`text-[11px] sm:text-xs font-medium ${row.enabled ? "text-gray-800" : "text-gray-400"}`}>
                    {row.enabled ? "Enabled" : "Disabled"}
                  </span>
                </div>
              </td>
              <td className="px-3 sm:px-4 py-2.5 sm:py-3.5 text-[11px] sm:text-xs font-medium text-gray-800 whitespace-nowrap">{row.threshold}</td>
              <td className="px-3 sm:px-4 py-2.5 sm:py-3.5">
                {row.rule === "–" ? (
                  ...
                  <div>
                    {row.rule.split("\n").map((line, j) => (
                      <div key={j} className={`text-[11px] sm:text-xs ${j === 0 ? "text-gray-800 font-medium" : "text-gray-800"}`}>{line}</div>
                    ))}
                  </div>
                )}
              </td>
              <td className="px-3 sm:px-4 py-2.5 sm:py-3.5 text-[11px] sm:text-xs font-medium text-gray-800 whitespace-nowrap">{row.lastPayout}</td>
              <td className="px-3 sm:px-4 py-2.5 sm:py-3.5"><StatusBadge active={row.active} /></td>
              <td className="px-3 sm:px-4 py-2.5 sm:py-3.5"><ThreeDotMenu /></td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div className="min-h-screen p-3 sm:p-0">
      ...
      <div className="mb-4 sm:mb-5">
        <h1 className="text-lg sm:text-xl font-bold text-gray-900">Auto Payout</h1>
        <p className="text-[11px] sm:text-sm text-gray-500 mt-0.5">Configure auto payout settings for merchants and payout APIs.</p>
      </div>

      {/* Tabs */}
      <div className="flex border-b border-gray-200 mb-4 sm:mb-5">
        <button
          onClick={() => setActiveTab("merchant")}
          className={`px-3 sm:px-5 py-2 sm:py-2.5 text-[11px] sm:text-xs font-semibold border-b-2 transition-colors -mb-px ${activeTab === "merchant" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
        >
          By Merchant
        </button>
        <button
          onClick={() => setActiveTab("api")}
          className={`px-3 sm:px-5 py-2 sm:py-2.5 text-[11px] sm:text-xs font-semibold border-b-2 transition-colors -mb-px ${activeTab === "api" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
        >
          By Payout API
        </button>
      </div>
      ...
      {/* Info Banner */}
      <div className="flex items-start gap-2 bg-blue-50 border border-blue-100 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 mb-4 sm:mb-5 text-[11px] sm:text-xs text-blue-700">
        <svg width={14} height={14} viewBox="0 0 24 24" fill="currentColor" className="text-blue-500 flex-shrink-0 mt-0.5">
          <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z" />
        </svg>
      ...
      {/* Note Banner */}
      <div className="flex items-start gap-2 bg-blue-50 border border-blue-100 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-blue-700">
        <svg width={14} height={14} viewBox="0 0 24 24" fill="currentColor" className="text-blue-500 flex-shrink-0 mt-0.5">
          <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z" />
        </svg>
        <div>
          <span className="font-semibold text-blue-800 text-[11px] sm:text-xs">Note</span>
          <p className="text-blue-600 mt-0.5 text-[11px] sm:text-xs leading-relaxed">Auto payout will be triggered automatically based on the configured threshold and rules.</p>
        </div>
      </div>
    </div>
  );
