...
// ── Status Badge ────────────────────────────────────────────────────────────
const StatusBadge = ({ status }) => (
  <span className={`px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-semibold border ${
    status === "Active"
      ? "bg-green-50 text-green-700 border-green-200"
      : "bg-red-50 text-red-600 border-red-200"
  }`}>
...
const ApiChip = ({ name, label }) => {
  const c = apiColors[name] || { bg: "bg-gray-100", text: "text-gray-600", border: "border-gray-200" };
  return (
    <span className={`inline-flex items-center px-1.5 sm:px-2 py-0.5 rounded text-[9px] sm:text-[11px] font-semibold border ${c.bg} ${c.text} ${c.border} whitespace-nowrap`}>
      <span className="hidden sm:inline">{name}</span>
      <span className="sm:hidden">{name.charAt(0)}</span>
      {label && <span className="ml-0.5 sm:ml-1 font-normal opacity-75">{label}</span>}
    </span>
  );
};
...
// ── Stat Card ───────────────────────────────────────────────────────────────
const StatCard = ({ icon, label, value, sub, subColor }) => (
  <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 px-2 sm:px-3 py-2 flex items-center gap-2 sm:gap-4 flex-1 min-w-0">
    <div className="w-8 h-8 sm:w-11 sm:h-11 rounded-xl bg-blue-50 flex items-center justify-center shrink-0 text-blue-500">
      {icon}
    </div>
    <div className="min-w-0 flex-1">
      <div className="text-[10px] sm:text-xs font-medium text-gray-500 mb-0.5 truncate">{label}</div>
      <div className="text-base sm:text-xl font-bold text-gray-900 leading-tight truncate">{value}</div>
      {sub && <div className={`text-[10px] sm:text-xs font-semibold mt-0.5 ${subColor || "text-gray-400"} truncate`}>{sub}</div>}
    </div>
  </div>
);
...
const Select = ({ options, defaultValue }) => (
  <div className="relative w-full sm:w-auto">
    <select className="w-full appearance-none pl-2.5 sm:pl-3 pr-6 sm:pr-8 py-1.5 sm:py-2 text-[11px] sm:text-xs border border-gray-200 rounded-lg bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 cursor-pointer">
      {options.map(o => <option key={o}>{o}</option>)}
    </select>
    <svg className="absolute right-2 sm:right-2.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" width={10} height={10} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M19 9l-7 7-7-7"/></svg>
  </div>
);
...
export default function PayoutRouting() {
  ...
  return (
    <div className="min-h-screen p-3 sm:p-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-4 sm:mb-5">
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-gray-900">Payout Routing</h1>
          <div className="flex flex-wrap items-center gap-1 text-[11px] sm:text-xs text-gray-500 mt-0.5">
            {["Dashboard", "Routing", "Routing List"].map((b, i, arr) => (
        ...
        <div className="flex gap-2">
          <button className="flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 border border-gray-200 rounded-lg text-[11px] sm:text-xs font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors shadow-sm">
            <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            <span className="hidden sm:inline">Routing Logs</span>
            <span className="sm:hidden">Logs</span>
          </button>
          <button className="flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-[11px] sm:text-xs font-semibold transition-colors shadow-sm">
      ...
      {/* Stat Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 mb-4 sm:mb-5">
        <StatCard icon={<RouteIcon />} label="Total Routing Rules" value="24" />
        <StatCard
          icon={<svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth={1.8}><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg>}
          label="Active Rules"
          value="18"
          sub="75.00%"
          subColor="text-green-600"
        />
        <StatCard
          icon={<svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="#ef4444" strokeWidth={1.8}><circle cx="12" cy="12" r="10"/><path d="M10 15V9M14 15V9"/></svg>}
          label="Inactive Rules"
          value="6"
          sub="25.00%"
          subColor="text-red-500"
      ...
      <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-2 sm:gap-3 px-3 sm:px-5 py-3 border-b border-gray-100">
          {/* Search */}
          <div className="relative flex-1 min-w-[200px]">
            <svg className="absolute left-2.5 sm:left-3 top-1/2 -translate-y-1/2 text-gray-400" width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
            <input
              value={search}
              onChange={e => setSearch(e.target.value)}
              className="w-full pl-7 sm:pl-9 pr-2.5 sm:pr-3 py-1.5 sm:py-2 text-[11px] sm:text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 placeholder:text-gray-400"
          ...
          <button className="flex items-center justify-center gap-1 px-2.5 sm:px-3 py-1.5 sm:py-2 border border-gray-200 rounded-lg text-[11px] sm:text-xs text-gray-600 hover:bg-gray-50 transition-colors sm:ml-auto">
            <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M1 4v6h6M23 20v-6h-6M20.49 9A9 9 0 005.64 5.64L1 10M23 14l-4.64 4.36A9 9 0 013.51 15"/></svg>
            Refresh
          </button>
        </div>
  ...
  <table className="w-full min-w-[900px]">
    <thead>
      <tr className="bg-gray-50/70 border-b border-gray-100">
        {["Routing Name", "Type", "Merchant Scope", "Payout APIs", "Status", "Default API", "Created On", ""].map(h => (
          <th key={h} className="px-3 sm:px-4 py-2.5 sm:py-3 text-left text-[10px] sm:text-xs font-semibold text-black whitespace-nowrap">{h}</th>
        ))}
      </tr>
    </thead>
    <tbody>
      {filtered.map((row) => (
        <tr key={row.name} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors group">
          <td className="px-3 sm:px-4 py-3">
            <span className="text-xs sm:text-sm font-medium text-blue-800 hover:underline cursor-pointer whitespace-nowrap">{row.name}</span>
          </td>
          <td className="px-3 sm:px-4 py-3 text-xs sm:text-sm text-gray-800 font-semibold whitespace-nowrap">{row.type}</td>
          <td className="px-3 sm:px-4 py-3 text-xs sm:text-sm text-gray-800 font-semibold whitespace-nowrap truncate max-w-[120px] sm:max-w-none">{row.scope}</td>
          <td className="px-3 sm:px-4 py-3">
          ...
          <td className="px-3 sm:px-4 py-3"><StatusBadge status={row.status} /></td>
          <td className="px-3 sm:px-4 py-3 text-xs sm:text-sm text-gray-800 font-semibold whitespace-nowrap">{row.default}</td>
          <td className="px-3 sm:px-4 py-3 text-[11px] sm:text-xs text-gray-800 font-semibold whitespace-nowrap">{row.created}</td>
          <td className="px-3 sm:px-4 py-3"><ThreeDot /></td>
        </tr>
      ))}
    </tbody>
  </table>
...
{/* Pagination */}
<div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-3 sm:px-5 py-3 border-t border-gray-100">
  <span className="text-[10px] sm:text-xs text-gray-500 text-center">
    Showing 1 to {filtered.length} of {routingData.length} results
  </span>
  <div className="flex flex-wrap items-center justify-center gap-2">
    <Select options={["10 per page", "25 per page", "50 per page"]} defaultValue="10 per page" />
    <div className="flex items-center gap-1">
      <button 
        onClick={() => setPage(p => Math.max(1, p - 1))}
        disabled={page === 1}
        className="p-1 rounded border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
      >
        <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
          <path d="M15 18l-6-6 6-6"/>
        </svg>
      </button>
      {Array.from({ length: Math.min(3, Math.ceil(routingData.length / 10)) }, (_, i) => i + 1).map(p => (
        <button
          key={p}
          onClick={() => setPage(p)}
          className={`w-7 h-7 sm:w-8 sm:h-8 rounded text-[11px] sm:text-xs font-medium transition-colors ${
      ...
      {Math.ceil(routingData.length / 10) > 3 && (
        <>
          <span className="text-gray-400 text-xs">...</span>
          <button
            onClick={() => setPage(Math.ceil(routingData.length / 10))}
            className={`w-7 h-7 sm:w-8 sm:h-8 rounded text-[11px] sm:text-xs font-medium transition-colors ${
              page === Math.ceil(routingData.length / 10) 
                ? "bg-blue-600 text-white" 
                : "border border-gray-200 text-gray-600 hover:bg-gray-50"
            }`}
          >
            {Math.ceil(routingData.length / 10)}
          </button>
        </>
      )}
      <button 
        onClick={() => setPage(p => Math.min(Math.ceil(routingData.length / 10), p + 1))}
        disabled={page === Math.ceil(routingData.length / 10)}
        className="p-1 rounded border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
      >
        <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
