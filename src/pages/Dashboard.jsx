...
function StatusBadge({ status }) {
    ...
    return (
        <span className={`px-1.5 sm:px-2 py-0.5 rounded-lg text-[10px] sm:text-xs font-semibold ${map[status] || "bg-gray-100 text-gray-600"}`}>
            {status}
        </span>
    );
}
...
function TransactionRow({ id, merchant, amount, status, time }) {
    return (
        <tr className="border-b border-gray-50 last:border-0">
            ...
            <td className="py-1.5 sm:py-2 text-[11px] sm:text-xs text-gray-600 font-medium whitespace-nowrap">{amount}</td>
            <td className="py-1.5 sm:py-2"><StatusBadge status={status} /></td>
...
function ApiRateBar({ name, rate, color }) {
    return (
        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 w-full">
            <span className="text-[11px] sm:text-xs font-medium text-gray-600 sm:min-w-20">
                {name}
            </span>
        ...
        <Card>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4">
                <div className="h-[120px] w-[120px] sm:h-[140px] sm:w-[140px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            <Pie data={payoutPie} cx="50%" cy="50%" innerRadius={35} outerRadius={55} dataKey="value" strokeWidth={2}>
                                {payoutPie.map((e, i) => <Cell key={i} fill={e.color} />)}
                            </Pie>
                        </PieChart>
                    </ResponsiveContainer>
...
function RecentTransactionsTable({ data }) {
    return (
        <Card>
            <SectionTitle action="View All">Recent Transactions</SectionTitle>
            <div className="overflow-x-auto">
                <table className="w-full min-w-[500px] sm:min-w-full text-[11px] sm:text-xs">
                    <thead>
                        <tr className="text-black border-b border-gray-100 bg-[#F7F8FA]">
                            {["Txn ID", "Merchant", "Amount", "Status", "Time"].map(h => (
...
function DashboardPage() {
    ...
    return (
        <div className="space-y-3 sm:space-y-4 p-3 sm:p-0">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                    <DateRangePicker 
                        ...
                        placeholder="14 May, 2025 - 14 May, 2025"
                    />
                    <button className="flex items-center justify-center gap-1.5 sm:gap-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg px-3 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs font-semibold">
                        <RefreshCw size={12} sm:size={13} /> Refresh
                    </button>
                </div>
            </div>

            {/* Top Metric Cards Row - Responsive grid */}
            ...
            {/* Bottom Metric Cards Row - Responsive grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
                {bottomCards.map((card, index) => (
                    <MetricCard key={index} {...card} />
                ))}
