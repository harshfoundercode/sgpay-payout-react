...
function Badge({ label, styleMap }) {
  return (
    <span className={`px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-medium whitespace-nowrap ${styleMap[label] ?? "bg-gray-100 text-gray-600"}`}>
      {label}
    </span>
  );
}
...
export default function MerchantListPage() {
  ...
  const statusOptions = ["All Status", "Active", "Inactive", "Pending Approval"];
    ...
    <div className="min-h-screen p-3 sm:p-0">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 p-3 sm:p-4 border-b border-gray-100">
          ...
          <button className="flex items-center justify-center gap-1 px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs text-gray-600 bg-gray-50 border border-gray-200 rounded-lg hover:bg-gray-100 transition-colors w-full sm:w-auto">
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
            </svg>
            Refresh
              ...
              {filtered.map((m, i) => (
                <tr key={m.id} className="border-b border-gray-50 hover:bg-blue-50/30 transition-colors">
                  <td className="px-3 sm:px-4 py-2 sm:py-3">
                    <button
                      onClick={() => navigate(`/merchants/${m.id}`)}
                      className="font-semibold text-gray-600 hover:underline whitespace-nowrap text-[11px] sm:text-xs"
                    >
                      {m.id}
                    </button>
                   </td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3 text-gray-800 font-medium whitespace-nowrap text-[11px] sm:text-xs max-w-[150px] truncate">{m.company}</td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3 text-gray-600 whitespace-nowrap text-[11px] sm:text-xs">{m.contact}</td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3 text-gray-600 whitespace-nowrap text-[11px] sm:text-xs">{m.mobile}</td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3 text-gray-500 whitespace-nowrap text-[11px] sm:text-xs max-w-[150px] truncate">{m.email}</td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3 text-gray-800 font-medium whitespace-nowrap text-[11px] sm:text-xs">{m.balance}</td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3 whitespace-nowrap"><Badge label={m.status} styleMap={STATUS_STYLES} /></td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3 text-gray-500 whitespace-nowrap text-[11px] sm:text-xs">{m.created}</td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-3 sm:px-4 py-3 border-t border-gray-100">
          <p className="text-[11px] sm:text-sm text-gray-500">Showing 1 to 10 of 1,248 results</p>
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-1.5">
            <div className="flex items-center gap-1.5 mr-0 sm:mr-3">
              <span className="text-[11px] sm:text-sm text-gray-500">10 per page</span>
              <select className="text-[11px] sm:text-sm text-gray-600 bg-gray-50 border border-gray-200 rounded px-2 py-1 focus:outline-none">
                <option>10</option><option>25</option><option>50</option>
              </select>
            </div>
            <button 
            ...
            <span className="text-gray-400 px-0.5 sm:px-1 text-[11px] sm:text-sm">...</span>
            <button 
              onClick={() => setPage(totalPages)}
              className="w-8 h-7 sm:w-10 sm:h-8 flex items-center justify-center rounded-lg text-[11px] sm:text-sm border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors"
            >
              {totalPages}
            </button>
            <button 
              onClick={() => setPage(p => Math.min(totalPages, p + 1))}
              className="w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 transition-colors text-sm sm:text-base"
            >
              ›
            </button>
          </div>
