...
const CustomLegend = () => (
  <div className="flex flex-col gap-2 ml-4">
    {[
      { label: "Success",  count: "25,430 (98.42%)", color: "#16a34a" },
      { label: "Failed",   count: "412 (1.58%)",     color: "#ef4444" },
      { label: "Pending",  count: "56 (0.22%)",      color: "#f59e0b" },
      { label: "Returned", count: "32 (0.12%)",      color: "#9ca3af" },
    ].map(item => (
      <div key={item.label} className="flex items-center gap-2">
        <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: item.color }} />
        <span className="text-xs text-gray-700 font-medium w-16">{item.label}</span>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              {["7D", "30D", "90D"].map(p => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-semibold transition-colors ${
                    chartPeriod === p ? "bg-blue-600 text-white" : "bg-white text-gray-600 hover:bg-gray-50"
                  }`}
                >
                  {p}
                </button>
          ...
          <ResponsiveContainer width="100%" height={230}>
            <LineChart data={volumeData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <YAxis tickFormatter={yFormatter} tick={{ fontSize: 11, fill: "#94a3b8" }} axisLine={false} tickLine={false} width={36} />
          ...
          <div className="flex items-center justify-center">
            <div className="relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-xl font-bold text-gray-900">25,842</span>
