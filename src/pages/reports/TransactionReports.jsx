...
// ── Stat Card ────────────────────────────────────────────────────────────────
const StatCard = ({ icon, iconBg, label, value, change, changeColor, sub }) => (
    <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 p-2.5 sm:p-4 flex items-start gap-2 sm:gap-3">
        <div className={`w-7 h-7 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0 ${iconBg}`}>
            {icon}
        </div>
        <div className="min-w-0 flex-1">
            <div className="text-[9px] sm:text-[11px] font-medium text-black mb-0.5 truncate">{label}</div>
            <div className="text-[13px] sm:text-[16px] font-bold text-gray-900 leading-tight">{value}</div>
            <div className="flex flex-wrap items-center gap-0.5 sm:gap-1 mt-0.5">
                <span className={`text-[10px] sm:text-xs font-semibold ${changeColor}`}>{change}</span>
                <span className="text-[10px] sm:text-[12px] text-gray-600 font-medium">{sub}</span>
            </div>
        </div>
    </div>
);
...
const Select = ({ options }) => (
    <div className="relative w-full sm:w-auto">
        <select className="w-full appearance-none pl-2 sm:pl-3 pr-5 sm:pr-7 py-1.5 sm:py-2 text-[11px] sm:text-xs border border-gray-200 rounded-lg bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 cursor-pointer">
            {options.map(o => <option key={o}>{o}</option>)}
        </select>
        <svg className="absolute right-1.5 sm:right-2 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" width={9} height={9} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M19 9l-7 7-7-7" /></svg>
    </div>
);
...
const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
        return (
            <div className="bg-white border border-gray-100 rounded-xl shadow-lg px-2 sm:px-3 py-1.5 sm:py-2 text-[10px] sm:text-xs">
                <div className="font-semibold text-gray-700 mb-0.5 sm:mb-1">{label}</div>
                <div className="text-blue-600 font-bold">{(payload[0].value / 1000000).toFixed(1)}M</div>
            </div>
        );
    }
...
const statCards = [
    ...
    },
    {
        iconBg: "bg-indigo-50",
        icon: (
            <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="#4f46e5" strokeWidth={1.8}>
                <path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8z M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75" />
            </svg>
        ),
        label: "Active Merchants", value: "872",
...
const CustomLegend = () => {
    ...
    return (
        <div className="ml-2 sm:ml-4 space-y-2 sm:space-y-4 min-w-[140px] sm:min-w-[180px]">
            {items.map((item) => (
                <div key={item.label} className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-[10px] sm:text-xs font-semibold text-gray-900 whitespace-nowrap">
                        {item.count}
                        <span className="text-gray-500 font-medium ml-0.5 sm:ml-1">
                            ({item.percent})
                        </span>
                    </span>
                </div>
    ...
    return (
        <div className="min-h-screen p-3 sm:p-0">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
                <div>
                    <h1 className="text-lg sm:text-xl font-bold text-gray-900">Reports Overview</h1>
                    <p className="text-[11px] sm:text-xs text-gray-500 mt-0.5">Overview of your payout platform performance</p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    <DateRangePicker 
                        onDateChange={handleDateChange}
                    ...
                    <Select options={["All APIs", "RazorpayX", "Cashfree", "Paytm"]} />
                    <Select options={["All Status", "Active", "Inactive"]} />
                    <button className="px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-600 hover:bg-blue-700 text-white text-[11px] sm:text-xs font-semibold rounded-lg transition-colors shadow-sm">
                        Apply
                    </button>
                    <button className="flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 border border-gray-200 rounded-lg text-[11px] sm:text-xs font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors shadow-sm">
                        ...
                        Export
                        <svg width={9} height={9} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M19 9l-7 7-7-7" /></svg>
                    </button>
                </div>
            </div>

            {/* ── Stat Cards Row 1 ── */}
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3 mb-2 sm:mb-3">
                {statCards.slice(0, 6).map((c) => <StatCard key={c.label} {...c} />)}
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-4 sm:gap-5 mb-4 sm:mb-5">
                ...
                {/* Line Chart */}
                <div className="flex-1 lg:flex-[2] bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3 sm:mb-4">
                        <h2 className="text-xs sm:text-sm font-bold text-gray-900">Transaction Volume Overview</h2>
                        <div className="flex rounded-lg overflow-hidden border border-gray-200 self-start">
                            {["7D", "30D", "90D"].map(p => (
                                <button
                                    key={p}
                                    onClick={() => setChartPeriod(p)}
                                    className={`px-2.5 sm:px-3 py-1 text-[11px] sm:text-xs font-semibold transition-colors ${chartPeriod === p ? "bg-blue-600 text-white" : "bg-white text-gray-600 hover:bg-gray-50"
                                        }`}
                                >
                                    {p}
                                </button>
                    ...
                    <div className="h-[200px] sm:h-[230px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={volumeData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                                ...
                                <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false} />
                                <XAxis dataKey="date" tick={{ fontSize: 9, fill: "#141342" }} axisLine={false} tickLine={false} />
                                <YAxis tickFormatter={yFormatter} tick={{ fontSize: 9, fill: "#141342" }} axisLine={false} tickLine={false} width={30} />
                                <Tooltip content={<CustomTooltip />} />
                                <Line
                                    type="monotone"
                                    dataKey="value"
                ...
                {/* Donut Chart */}
                <div className="flex-1 bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
                    <h2 className="text-xs sm:text-base font-semibold text-gray-900 mb-3 sm:mb-5">
                        Status Distribution
                    </h2>

                    <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
                        <div className="relative flex-shrink-0">
                            <div className="h-[140px] w-[140px] sm:h-[175px] sm:w-[175px]">
                                <ResponsiveContainer width="100%" height="100%">
                                    <PieChart>
                                        <Pie
                                            data={pieData}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-5">
                ...
                <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
                    <div className="flex items-center gap-2 mb-3 sm:mb-4 pb-2 sm:pb-3 border-b border-gray-50">
                        <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl bg-purple-50 flex items-center justify-center">
                            <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="#7c3aed" strokeWidth={1.8}>
                                <rect x="1" y="4" width="22" height="16" rx="2" /><path d="M1 10h22" />
                            </svg>
                        </div>
                        <h3 className="text-xs sm:text-sm font-bold text-gray-900">Merchant Summary</h3>
                    </div>
                    <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-3 sm:mb-4">
                        {[
                            ...
                            <div key={s.label} className="text-center">
                                <div className="text-sm sm:text-base font-bold text-gray-900">{s.val}</div>
                                <div className="text-[10px] sm:text-[11px] text-gray-500 mt-0.5 leading-tight">{s.label}</div>
                            </div>
                        ))}
                    </div>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-2 sm:pt-3 border-t border-gray-50">
                        <span className="text-[11px] sm:text-xs text-green-600 font-semibold">↑ 8 vs last 30 days</span>
                        <button className="flex items-center justify-center gap-1 text-[11px] sm:text-xs font-semibold text-blue-600 hover:text-blue-700 transition-colors">
                            View Report
                ...
                <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
                    <div className="flex items-center gap-2 mb-3 sm:mb-4 pb-2 sm:pb-3 border-b border-gray-50">
                        <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl bg-blue-50 flex items-center justify-center">
                            <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="#3b82f6" strokeWidth={1.8}>
                                <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" />
                            </svg>
                        </div>
                        <h3 className="text-xs sm:text-sm font-bold text-gray-900">API Summary</h3>
                    </div>
                    <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-3 sm:mb-4">
                        {[
                            { label: "Active APIs", val: "5" },
                            ...
                            <div key={s.label} className="text-center">
                                <div className="text-sm sm:text-base font-bold text-gray-900 leading-tight">{s.val}</div>
                                <div className="text-[10px] sm:text-[11px] text-gray-500 mt-0.5 leading-tight">{s.label}</div>
                            </div>
                        ))}
                    </div>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-2 sm:pt-3 border-t border-gray-50">
                        <span className="text-[11px] sm:text-xs text-green-600 font-semibold">↑ 0.65% vs last 30 days</span>
                        <button className="flex items-center justify-center gap-1 text-[11px] sm:text-xs font-semibold text-blue-600 hover:text-blue-700 transition-colors">
                            View Report
                            <svg width={11} height={11} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}><path d="M5 12h14M12 5l7 7-7 7" /></svg>
                        </button>
                ...
                <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
                    <div className="flex items-center gap-2 mb-3 sm:mb-4 pb-2 sm:pb-3 border-b border-gray-50">
                        <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl bg-green-50 flex items-center justify-center">
                            <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth={1.8}>
                                <path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z" /><polyline points="9 22 9 12 15 12 15 22" />
                            </svg>
                        </div>
                        <h3 className="text-xs sm:text-sm font-bold text-gray-900">Settlement Summary</h3>
                    </div>
                    <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-3 sm:mb-4">
                        {[
                            { label: "Settlement Amount", val: "₹11.80 Cr" },
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-2 sm:pt-3 border-t border-gray-50">
                        <span className="text-[11px] sm:text-xs text-green-600 font-semibold">↑ 16.2% vs last 30 days</span>
                        <button className="flex items-center justify-center gap-1 text-[11px] sm:text-xs font-semibold text-blue-600 hover:text-blue-700 transition-colors">
                            View Report
                            <svg width={11} height={11} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}><path d="M5 12h14M12 5l7 7-7 7" /></svg>
                        </button>
